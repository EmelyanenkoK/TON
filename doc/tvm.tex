...
\item Finally, each of the $r$ cell references is represented by 2 bytes encoding $\Depth(c_i)$ explained above in~\ptref{sp:cell.depth} and 32 bytes containing the 256-bit {\em representation hash\/} $\Hash(c_i)$, explained below in~\ptref{sp:repr.hash}, of the cell $c_i$ referred to.
\end{enumerate}
